    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          <h1 className="text-3xl md:text-4xl font-black text-slate-950 uppercase tracking-tighter flex flex-wrap items-center gap-3">
            BLOG <span className="text-[#081ff0] italic">MANAGEMENT</span>
      ...
      <div className="relative group">
        <div className="relative overflow-hidden bg-white border border-slate-200 rounded-3xl p-6 text-slate-900 min-h-[400px] shadow-sm">
          {loading ? (
            <div className="flex items-center justify-center h-64 text-slate-400">Loading posts...</div>
          ) : error ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center space-y-4">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold uppercase tracking-tight text-slate-950">No blog posts found</h3>
                <p className="text-slate-600 text-sm max-w-xs mx-auto">You don't have any blog posts yet. Click the create button to get started.</p>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:hidden">
                {paginatedPosts.map((post) => (
                  <div key={post.id} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4 hover:shadow-md transition-all duration-300">
                    <div className="flex items-start gap-4">
                      <div className="relative group/img shrink-0">
                        <img src={post.image} alt={post.title} className="relative w-20 h-20 rounded-xl object-cover border border-slate-200" />
                      </div>
                      <div className="flex-1 min-w-0 space-y-1">
                        ...
                        <h3 className="font-bold text-sm leading-tight text-slate-900 line-clamp-2">{post.title}</h3>
                      ...
                      <tr key={post.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <img src={post.image} alt={post.title} className="w-12 h-12 rounded-lg object-cover border border-slate-100" />
                            <div>
                              <p className="font-bold text-sm text-slate-900 line-clamp-1">{post.title}</p>
                              <p className="text-xs text-slate-400 line-clamp-1">{post.slug}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                          <span className="px-2 py-1 bg-slate-100 rounded text-[10px] font-black uppercase text-slate-600">{post.category}</span>
                        </td>
                        <td className="p-4">
